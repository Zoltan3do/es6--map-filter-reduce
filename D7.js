...
let concatenazioneStringhe = (a, b) => (a.substring(0, 2) + b.substring(b.length - 3)).toUpperCase();
...
let somma = randomArray().reduce((s, item) => s += item);
console.log(somma);
...
function olderFilm(arr) {
  let older = 2025;
  arr.forEach((item) => {
    if (Number(item.Year) < older)
      older = Number(item.Year);
  });
  return older;
}
...
let filmMillennials = movies.filter(item => item.Year.substring(0, 1) === "2");
